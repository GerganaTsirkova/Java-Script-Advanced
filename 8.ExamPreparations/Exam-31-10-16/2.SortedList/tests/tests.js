...
describe("SortedList", function() {
    ...
    it("should have all of the functions in it's prototype", function() {
        expect(myList).to.be.an.instanceOf(SortedList);
        expect(myList).to.have.property("add");
        expect(myList).to.have.property("remove");
        expect(myList).to.have.property("get");
        expect(myList).to.have.property("sort");
        expect(myList).to.have.property("vrfyRange");
        expect(myList).to.have.property("size");
        expect(myList).to.have.property("list");
    ...
    it("should be sorted after adding", function() {
        myList.add(5);
        expect(myList.list[0]).to.equal(5, "Element wasn't added.");
        myList.add(2);
        expect(myList.list[0]).to.equal(2, "Collection wasn't sorted.");
        expect(myList.list[1]).to.equal(5, "Collection wasn't sorted.");
    });
